...
namespace V_verPlatform.Models.Novel
{
    public class NovelService
    {
        static public List<NovelomitData> RetList()
        {
            ...
            SqlDataReader sqdr = SqlHelper.ExecuteReader(SqlHelper.conStr, CommandType.Text, "SELECT * FROM novel ORDER BY ID DESC");
                while(sqdr.Read())
                {
        ...
        {
            List<NovelomitData> list = new List<NovelomitData>();
            SqlDataReader sqdr = SqlHelper.ExecuteReader(SqlHelper.conStr, CommandType.Text, "SELECT * FROM novel ORDER BY ID DESC WHERE classify='"+classify+"'");
            while (sqdr.Read())
            {
                list.Add(new NovelomitData((DateTime)sqdr["Date"], (int)sqdr["userID"], (String)sqdr["classify"], (String)sqdr["title"], (int)sqdr["ID"]));
            }
        ...
        {
            NovelData nd;
            SqlDataReader sqdr = SqlHelper.ExecuteReader(SqlHelper.conStr, CommandType.Text, "SELECT * FROM novel WHERE ID="+ID);
             if (sqdr.Read())
             {
        ...
        {
            NovelData nd;
            SqlDataReader sqdr = SqlHelper.ExecuteReader(SqlHelper.conStr, CommandType.Text, "SELECT * FROM novel WHERE title='"+title+"'");
             if (sqdr.Read())
             {
             nd= new NovelData((DateTime)sqdr["Date"],(String)sqdr["text"],(int)sqdr["userID"], (String)sqdr["classify"], (String)sqdr["title"], (int)sqdr["ID"]);
              }
